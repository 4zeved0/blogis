    ...
    <main className="min-h-screen bg-background text-text px-6 py-12 font-sans">
      <div className="max-w-3xl mx-auto">
        <div className='flex justify-between'>
          <Link
            href="/"
            className="bg-surface rounded p-3 text-secondary text-sm font-medium hover:underline hover:text-primary/80 transition mb-6 inline-block"
          >
          ...
          <div>
            {!session && (
              <button
                onClick={() => signIn()}
                className="sm:border-primary sm:border-2 bg-primary hover:bg-primary/80 text-white px-5 py-2 rounded transition-all duration-200 focus:ring-2 focus:ring-primary sm:justify-self-end"
              >
                Fazer Login
              </button>
            )}
        ...
        {post.mainImage?.asset?._ref && (
          <img
            ...
            alt={post.title}
            className="w-full h-64 object-cover rounded mb-8 shadow-md"
          />
        )}
